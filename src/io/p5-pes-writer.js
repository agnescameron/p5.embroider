...
pecEncode(stitches, colors, trimFlags) {
  ...
  for (let i = 0; i < stitches.length; i++) {
    ...
    // Check if we need long form (large delta or special command)
    const needsLongForm = (dx >= 63 || dx <= -64 || dy >= 63 || dy <= -64 || isTrim);
    
    // First stitch is always written but not necessarily as trim
    if (i === 0) {
      // Always use long form for first stitch
      dx = this.encodeLongForm(dx);
      dy = this.encodeLongForm(dy);
      
      // Only flag as trim if it actually is one
      if (isTrim) {
        dx = this.flagTrim(dx);
        dy = this.flagTrim(dy);
